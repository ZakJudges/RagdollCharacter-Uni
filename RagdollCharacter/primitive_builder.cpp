...
void PrimitiveBuilder::Init()
{
	...
	default_capsule_mesh_ = CreateCapsuleMesh(10, 10, 10.0f, 10.0f);
...
gef::Mesh * PrimitiveBuilder::CreateCapsuleMesh(int slices, int segments, float radius, float halfHeight, gef::Material* material)
{
	float dTheta = 3.1416 / (slices * 2);
	float dPhi = 6.2832 / segments;

	//float radius = radius_ * 0.02f;
	//float halfHeight = halfHeight_ * 0.02f;
	...
	for (int i = 0; i <= 2 * slices + 1; ++i)
	{
		for (int j = 0; j <= segments; ++j)
		{
			...
			gef::Vector4 position = gef::Vector4(x, y, z) * radius + gef::Vector4(0.0f, yoffset, 0.0f);
			gef::Vector4 normal(x, y, z);
			...
			vertex.ny = normal.y();
			vertex.nz = normal.z();
			vertex.u = 0.0f;	//change.
			vertex.v = 0.0f;	//change.
	...
	mesh->InitVertexBuffer(platform_, &vertices[0], vertices.size(), sizeof(gef::Mesh::Vertex));
	mesh->AllocatePrimitives(1);

	gef::Primitive* primitive = mesh->GetPrimitive(0);
	primitive->InitIndexBuffer(platform_, &indices[0], indices.size(), sizeof(Int32));
	primitive->set_type(gef::TRIANGLE_LIST);
