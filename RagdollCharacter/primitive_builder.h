...
public:
	...
	gef::Mesh* CreateBoxMesh(const gef::Vector4& half_size, gef::Vector4 centre = gef::Vector4(0.0f, 0.0f, 0.0f), gef::Material** materials = nullptr);
	...
	/// @param[in] materials	Pointer to material used to render all faces. nullptr is valid.
	gef::Mesh* CreateSphereMesh(const float radius, const int phi, const int theta, gef::Vector4 centre = gef::Vector4(0.0f, 0.0f, 0.0f), gef::Material* material = nullptr);

	gef::Mesh* CreatePlaneMesh(const gef::Vector4& half_size, gef::Vector4 centre = gef::Vector4(0.0f, 0.0f, 0.0f), gef::Material* material = nullptr);

	gef::Mesh* CreateCapsuleMesh(int slices, int segments, float radius, float halfHeight, gef::Material* material = nullptr);
	...
	/// @return The mesh for the default sphere.
	...
	/// @brief Get the default plane mesh.
	/// @return The mesh for the default plane.
	/// @note The default plane is in the XZ plane with size 10 with the centre at 0, 0, 0.
	inline const gef::Mesh* GetDefaultPlaneMesh() const {
		return default_plane_mesh_;
	};

	inline const gef::Mesh* GetDefaultCapsuleMesh() const {
		return default_capsule_mesh_;
	}
...
protected:
	...
	gef::Mesh* default_capsule_mesh_;
